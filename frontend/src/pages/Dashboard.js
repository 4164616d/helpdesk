    ...
    <div className="App-Main">
      ...
      <Stack direction="row" spacing={2}>
        {filters.map((filter,index) =>
          <Button variant={currentFilter === index ? "outlined" : "contained"} onClick={() => setCurrentFilter(index)}>{filter.name}</Button>
        )}
      </Stack>
      {filters[currentFilter].items.length>0?<Grid style={{ padding: "2% 10% 2% 10%" }} container spacing={2}>
        {filters[currentFilter].items.map(ticket =>
          <Grid item xs={6}>
